 ...
 * ------------------------------------------------------------- ;
  ...
  %MACRO PROG_EXE(PATHNAME,EXE_FMT,EXE_MSR,EXE_HOBS,EXE_HSTR,EXE_HRSK,EXE_AOBS,EXE_ARSK);
    %if %sysfunc(CEXIST(LIBRARY.FORMATS.IMMUNIP.FORMATC)) = 0 or &EXE_FMT. = 1 %then %do;
        %include PROGRMS(PDI_ALL_FORMATS.SAS);
    %end;
  
   %if %sysfunc(CEXIST(LIBRARY.FORMATS.IMMUNIP.FORMATC)) = 1 AND &EXE_MSR. = 1  %then %do;
    	%include PROGRMS(PDI_ALL_MEASURES.SAS) /source2;
   %end;

   %if %sysfunc(CEXIST(LIBRARY.FORMATS.IMMUNIP.FORMATC)) = 0 AND &EXE_MSR. = 1  %then %do;
  ...
  do i = 2 to &NDX.;
    if put(DX{i},&FMT.) = '1' then
      if DXPOA{i} IN ('N','U',' ','E','1','X') then
        if (ICDVER = 42 AND put(DX{i},$poaxmpt_v42fmt.)='0') OR
           (ICDVER = 41 AND put(DX{i},$poaxmpt_v41fmt.)='0') OR
        ...
        if put(DX{i},&FMT.) = '1' then do;
          if DXPOA{i} IN ('Y','W') then
            if (ICDVER = 42 AND put(DX{i},$poaxmpt_v42fmt.)='0') OR
               ...
               (ICDVER = 33 AND put(DX{i},$poaxmpt_v33fmt.)='0')
            then result = 1;
          if (ICDVER = 42 AND put(DX{i},$poaxmpt_v42fmt.)='1') OR
